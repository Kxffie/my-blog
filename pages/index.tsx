...
import { ChevronRight, ChevronLeft, Dice5 } from 'akar-icons';
    ...
    return (
      <div>
        ...
        <div className="flex justify-between mx-2 mt-6">
          <button onClick={() => setPage(page - 1)} disabled={page === 1} className='px-2 py-1 text-lg font-semibold text-white rounded-md shadow-lg bg-violet-500 disabled:bg-gray-500 disabled:text-gray-400'>
            <ChevronLeft size={36} />
          </button>
          <button onClick={() => setPage(page + 1)} disabled={page * itemsPerPage >= filteredPosts.length} className='px-2 py-1 ml-2 text-lg font-semibold text-white rounded-md shadow-lg bg-violet-500 disabled:bg-gray-500 disabled:text-gray-400'>
